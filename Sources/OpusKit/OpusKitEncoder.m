#import "include/OpusKitEncoder.h"
...
const NSInteger OpusKitEncoderSampleRate = 48000;
...
@interface FileDataItem : DataItem
...
@interface OpusKitEncoder ()
{
    AVAssetReader *_assetReader;
    AVAssetReaderOutput *_readerOutput;
    
    NSMutableData *_audioBuffer;
    FileDataItem *_tempFileItem;
    OggOpusWriter *_oggWriter;
    
    int _tailLength;
}
@end

@implementation OpusKitEncoder
...
- (instancetype)initWithURL:(NSURL *)url
{
    ...
    if (self != nil)
    {
        ...
        NSDictionary *outputSettings = @
        {
            ...
            AVSampleRateKey: @(OpusKitEncoderSampleRate),
        ...
        _tempFileItem = [[FileDataItem alloc] initWithTempFile];
    }
    return self;
}
...
+ (ATQueue *)processingQueue
{
    ...
    {
        queue = [[ATQueue alloc] initWithName:@"opusKit.opusAudioEncoderQueue"];
    });
...
{
    [[OpusKitEncoder processingQueue] dispatch:^
    {
        _oggWriter = [[OggOpusWriter alloc] init];
        ...
        [[OpusKitEncoder processingQueue] dispatch:^
        {
            FileDataItem *dataItemResult = nil;
            ...
            //Log(@"[OpusKitEncoder#%x convert time: %f ms]", self, (CFAbsoluteTimeGetCurrent() - startTime) * 1000.0);
